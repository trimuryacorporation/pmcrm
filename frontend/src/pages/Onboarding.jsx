...
function ProfileFields({ type, form, setValue }) {
  const common = <><Input label="Email address" name="email" type="email" required form={form} setValue={setValue} /><Input label="LinkedIn profile (optional)" name="linkedinUrl" form={form} setValue={setValue} /></>;
  const setLanguages = (field, languages) => {
    setValue(field, languages);
    setValue('languageTeamCounts', (form.languageTeamCounts || []).filter((item) => languages.includes(item.language)));
  };
