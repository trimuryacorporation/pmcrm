...
export async function listPortals(req, res, next) {
  try {
    ...
    const allowedFields = ['companyName', 'contactPerson', 'email', 'url'];
    const filter = {};
    if (String(q).trim()) {
...
export async function createPortal(req, res, next) {
  try {
    ...
    if (!companyName || !contactPerson || !/^\S+@\S+\.\S+$/.test(email)) {
      res.status(400);
      throw new Error('Company name, contact person name, and a valid email address are required');
    }
    const portal = await Portal.create({ url, companyName, contactPerson, email, createdBy: req.user._id });
    await writeAudit(req, 'CREATE', 'Portal', portal, { url, companyName, contactPerson, email });
    res.status(201).json({ message: 'Portal added successfully', item: portal });
  } catch (error) {
    next(error);
  }
}
